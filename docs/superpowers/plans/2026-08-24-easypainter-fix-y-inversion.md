...
- **交付后复验（用户报「还是颠倒」）**：根因是**跑的是陈旧 debug 二进制**——当时流水线只重编了 `release` 预设，`build/debug/easypainter`（18:28 构建）仍内嵌旧 shader（02:17 的 SPV）。处置：重建 `debug` 预设（新 shader 5672B）、kill 陈旧进程、在 `DISPLAY=:99`（Xvnc）重启修复后 debug 二进制，并用 **XTEST 真机注入**验证——窗口顶部区域(y≈96)拖拽，红色笔画全部落在上半部（minRow=96 maxRow=111，上=64 下=0）→ 修复在真实窗口路径生效。**教训：交付修复后须核对用户实际运行的二进制/进程是否陈旧（`pgrep` + 构建时间戳 + debug/release 预设），不能只验证 release。**
